...
export const EatTab = ({ dailyState, setDailyState, points, setPoints, cooldowns, setCooldowns, dayOfWeek }) => {
    const getWeightedRandomMeal = (excludeId) => {
        let pool = MEAL_POOL.filter(m => m.id !== excludeId);
        const totalWeight = pool.reduce((acc, m) => acc + (m.weight || 0), 0);
        let random = Math.random() * totalWeight;
        for (let meal of pool) {
            if (random < (meal.weight || 0))
                return meal;
            random -= (meal.weight || 0);
        }
        return pool[0];
    };
    const updateDaily = (key, value) => {
        setDailyState(prev => ({ ...prev, [key]: value }));
    };
    const handleRoll = (slot) => {
        ...
        updateDaily(`${slot}Patch`, null);
        updateDaily(`${slot}Failed`, false);
    };
    const toggleDorm = (slot) => {
    ...
    const handleRedeem = (rewardId) => {
        ...
        if (shouldReplaceLunch) {
            updateDaily('lunch', reward);
            updateDaily('lunchPatch', null);
            updateDaily('lunchFailed', false);
        }
        else {
            updateDaily('dinner', reward);
            updateDaily('dinnerPatch', null);
            updateDaily('dinnerFailed', false);
        }
    ...
    return (<div className="space-y-6 animate-in fade-in">
            ...
            <div>
                ...
                {dailyState.lunch || dailyState.lunchFailed ? (<MealCard meal={dailyState.lunch} isLocked={dailyState.lunch?.type === 'SSR_LOCK'} onRoll={() => handleRoll('lunch')} onToggleDorm={() => toggleDorm('lunch')} patch={dailyState.lunchPatch} onPatchChange={(p) => updateDaily('lunchPatch', p)} isFailed={dailyState.lunchFailed} onFail={() => updateDaily('lunchFailed', !dailyState.lunchFailed)}/>) : (<div onClick={() => handleRoll('lunch')} className="h-24 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl flex items-center justify-center text-gray-400 dark:text-gray-500 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            ...
            <div>
                ...
                {dailyState.dinner || dailyState.dinnerFailed ? (<MealCard meal={dailyState.dinner} isLocked={dailyState.dinner?.type === 'SSR_LOCK'} onRoll={() => handleRoll('dinner')} onToggleDorm={() => toggleDorm('dinner')} patch={dailyState.dinnerPatch} onPatchChange={(p) => updateDaily('dinnerPatch', p)} isFailed={dailyState.dinnerFailed} onFail={() => updateDaily('dinnerFailed', !dailyState.dinnerFailed)}/>) : (<div onClick={() => handleRoll('dinner')} className="h-24 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl flex items-center justify-center text-gray-400 dark:text-gray-500 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
            ...
            return (<button key={r.id} onClick={() => handleRedeem(r.id)} disabled={!canAfford || onCD} className={`p-3 rounded-xl border-2 text-left relative overflow-hidden transition-all active:scale-95 ${!canAfford || onCD ? 'bg-gray-50 dark:bg-gray-800 opacity-60' : 'bg-white dark:bg-gray-800 border-purple-100 dark:border-purple-900 hover:border-purple-300 dark:hover:border-purple-700'}`}>
