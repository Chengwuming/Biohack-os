...
import { Download, TrendingUp, Award, Flame, Zap as ZapIcon, FileText } from 'lucide-react';
...
export const StatsTab: React.FC<StatsTabProps> = ({ logs, points }) => {
    ...
    const handleMarkdownExport = () => {
        const trainingLogs = logs.filter(l => l.type === '训练');
        let md = `# BioHackOS 训练历史报告\n\n生成时间: ${new Date().toLocaleString()}\n\n`;
        md += "| 日期 | 训练详情 | 积分 |\n";
        md += "| :--- | :--- | :--- |\n";

        trainingLogs.forEach(log => {
            md += `| ${log.date} | ${log.details} | ${log.score} |\n`;
        });

        md += "\n\n---\nGenerated by BioHackOS AI Engineer";

        const blob = new Blob([md], { type: "text/markdown" });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `training_history_${new Date().toISOString().split('T')[0]}.md`;
        document.body.appendChild(a);
        a.click();
        document.body.removeChild(a);
        URL.revokeObjectURL(url);
        ...
        <div className="space-y-4 animate-in slide-in-from-right-4">
            ...
            <div className="flex justify-between items-end mb-4">
                ...
                <div className="flex items-center space-x-2">
                    <button onClick={handleMarkdownExport} className="text-xs flex items-center bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2 py-1 rounded hover:bg-purple-100 dark:hover:bg-purple-900/50 transition-colors">
                        <FileText size={12} className="mr-1" /> AI 报告
                    </button>
