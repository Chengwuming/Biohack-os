...
export const TrainTab = ({ workoutWeights, setWorkoutWeights, setPoints, setLogs }) => {
    ...
    return (<div className="space-y-6 animate-in slide-in-from-right-4">
            <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 shadow-sm transition-colors">
                ...
                <div className="space-y-4">
                    {['bench', 'squat', 'pull'].map(type => (<div key={type} className="flex justify-between items-center border-b border-gray-50 dark:border-gray-700 pb-2">
                            <label className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase w-24">
                                {type === 'bench' ? '卧推' : type === 'squat' ? '深蹲' : '下拉'}
                            </label>
                            <div className="flex items-center space-x-2">
                                <input type="number" value={workoutWeights[type]} onChange={(e) => setWorkoutWeights({ ...workoutWeights, [type]: parseInt(e.target.value) || 0 })} className="w-20 p-2 border border-gray-200 dark:border-gray-600 rounded-lg font-mono text-center font-bold bg-gray-50 dark:bg-gray-700 text-gray-800 dark:text-gray-100 outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"/>
