...
export const MealCard: React.FC<MealCardProps> = ({ meal, isLocked, onRoll, onToggleDorm, patch, onPatchChange, onFail, isFailed }) => {
    if (!meal) return <div className="p-8 text-center text-gray-400 border-2 border-dashed rounded-xl">待机中...</div>;

    if (isFailed) {
        return (
            <div className="relative bg-gray-100 rounded-xl border-2 border-gray-200 p-6 mb-4 opacity-60">
    ...
    return (
        <div className={`relative bg-white rounded-xl shadow-sm overflow-hidden mb-4 transition-all ${borderClass}`}>
                ...
                {!isDorm && !isLocked && !isReward && meal.patchRequired && (
                    <div className="mt-3 bg-red-50 p-2 rounded-lg border border-red-100">
                      ...
                      ${patch?.id === p.id ? 'bg-red-600 border-red-600 text-white shadow-md' : 'bg-white border-red-100 text-gray-600 hover:bg-red-50'}`}>
                        ...
                        {patch && (
                            <div className="bg-white p-2 rounded border border-red-100 text-xs text-red-800 font-bold flex items-start animate-in slide-in-from-top-1">
